...
public interface OswService {
     ...
     */
    public boolean connect(String server, Integer port, Map<String, String> parameters)
     ...
     * @return true if it succeeded in registering the user, i.e. receives a reply IQ of type
     *   RESULT.
     * @throws ConnectionRequired will throw an exception if the service was not connected.
     */
    public boolean register(String username, String password, String name, String email)
        throws ConnectionRequired;

    /**
     * Authenticates the user at the connected XMPP Server and tries to retrieve its roster and
     * its inbox of Activities.
     *
     ...
     * @throws ConnectionRequired will throw a ConnectionRequired exception if the service was not
     *   connected.
     * @throws RequestException will throw a RequestException if the XMPP connection fails to login
     *   the user.
     */
    ...
    /**
     * @return true if the XMPP connection is not null (i.e. the connection to XMPP Server has been
     *   established) and a user has already logged in by using the login method. Returns false
     *   otherwise.
     */
    ...
    /**
     * @return the jabberId of the user that was authenticated using the login method. Returns null
     *   if no user was authenticated yet.
     */
    public String getUser();

    /**
     * @return the host name of the XMPP Server currently connected to. Returns null if currently
     *   not connected.
     */
    public String getHostname();

    /**
     * Adds a listener to the connection that will be notified when the state of the connection
     * (connected, disconnectedOnError or disconnected) changes.
     *
     * @param listener
     */
    ...
    /**
     * Subscribe to the activity stream of another user, who will be added to the roster.
     * ("Follow").
     *
     * @param userJid jid of the user to subscribe to
     * @return true if it succeeds, i.e. receives a reply IQ of type RESULT.
     * @throws RequestException throws a RequestException if it receives a reply IQ of type ERROR
     *   after attempting to send a subscription IQ to the server.
     * @throws ConnectionRequired throws a ConnectionRequired exception if not connected to an XMPP
     *   Server.
     * @throws AuthenticationRequired throws an AuthenticationRequired is no user has been logged in
     *   yet.
     */
    public boolean subscribe(String userJid)
     ...
     * @return true if it succeeded in unsubscribing from a user, (i.e. if it receives a reply IQ of
     *   type RESULT.)
     * @throws RequestException throws a RequestException if it receives a reply IQ of type ERROR
     *   after attempting to send a unsubscription IQ to the server.
     * @throws ConnectionRequired throws a ConnectionRequired exception if not connected to an XMPP
     *   Server.
     * @throws AuthenticationRequired throws an AuthenticationRequired is no user has been logged in
     *   yet.
     */
    public boolean unsubscribe(String userJid)
        throws RequestException, ConnectionRequired, AuthenticationRequired;
     ...
     * @throws RequestException throws a RequestException if it receives a reply IQ of type ERROR
     *   after attempting to retrieve the list of subscribers from the server.
     * @throws ConnectionRequired throws a ConnectionRequired exception if not connected to an XMPP
     *   Server.
     * @throws AuthenticationRequired throws an AuthenticationRequired is no user has been logged in
     *   yet.
     */
    public List<String> getSubscribers(String jid)
        throws RequestException, ConnectionRequired, AuthenticationRequired;
     ...
     * @return a List of jids.
     * @throws RequestException throws a RequestException if it receives a reply IQ of type ERROR
     *   after attempting to retrieve the list of subscriptions from the server.
     * @throws ConnectionRequired throws a ConnectionRequired exception if not connected to an XMPP
     *   Server.
     * @throws AuthenticationRequired throws an AuthenticationRequired is no user has been logged in
     *   yet.
     */
    public List<String> getSubscriptions(String jid)
        throws RequestException, ConnectionRequired, AuthenticationRequired;

    /**
     * Get the OneSocialWeb Inbox encapsulated in the OswService
     *
     * @return returns the inbox of the service, which contains a list of activity entries. The
     *   inbox is refreshed upon actions such as OswService login and disconnect.
     */
    public Inbox getInbox();

    /**
     * Retrieves from the XMPP Server the List of Activities (ActivityEntries) of the specified
     *   user.
     *
     * @param userJid The jid of the user whose activities we want to retrieve.
     * @return A List of ActivityEntries.
     *   See: http://onesocialweb.org/spec/1.0/xep-osw-activities.html
     * @throws RequestException throws a RequestException if it receives from the server a reply IQ
     *   of type ERROR.
     * @throws ConnectionRequired throws a ConnectionRequired exception if not connected to an XMPP
     *   Server.
     * @throws AuthenticationRequired throws an AuthenticationRequired is no user has been logged in
    ...
    /**
     * @param entry An ActivityEntry.
     *   See: http://onesocialweb.org/spec/1.0/xep-osw-activities.html
     * @return true if it succeeded in posting the activity to the server (i.e. if it receives a
     *   reply IQ of type RESULT.)
     * @throws ConnectionRequired throws a ConnectionRequired exception if not connected to an XMPP
     *   Server.
     * @throws AuthenticationRequired throws an AuthenticationRequired is no user has been logged in
     *   yet.
     ...
     *   of type ERROR.
     */
    public boolean postActivity(ActivityEntry entry)
     ...
     *
     * @param activityId An ActivityId.
     *   See: http://onesocialweb.org/spec/1.0/xep-osw-activities.html
     * @return true if it succeeded in deleting the activity at the server (i.e. if it receives a
     *   reply IQ of type RESULT.)
     * @throws ConnectionRequired throws a ConnectionRequired exception if not connected to an XMPP
     *   Server.
     * @throws AuthenticationRequired throws an AuthenticationRequired is no user has been logged in
    ...
    /**
     * @param activityId An ActivityEntry.
     *   See: http://onesocialweb.org/spec/1.0/xep-osw-activities.html
     * @return true if it succeeded in updating the activity at the server (i.e. if it receives a
     *   reply IQ of type RESULT.)
     * @throws ConnectionRequired throws a ConnectionRequired exception if not connected to an XMPP
     *   Server.
     * @throws AuthenticationRequired throws an AuthenticationRequired is no user has been logged in
    ...
    /**
     * Retrieves from the XMPP server a complete profile by trying to send an IQ of type
     * IQProfileQuery.
     *
     * @param userJid jid of the user whose profile we want to retrieve.
     * @return the OneSocialWeb Profile.
     *   See: http://onesocialweb.org/spec/1.0/xep-osw-profile.html
     * @throws RequestException throws a RequestException if it receives from the server a reply IQ
     *   of type ERROR after trying to retrieve the Profile.
     * @throws ConnectionRequired throws a ConnectionRequired exception if not connected to an XMPP
     *   Server.
     * @throws AuthenticationRequired throws an AuthenticationRequired is no user has been logged in
     *   yet.
     ...
     *   See: http://onesocialweb.org/spec/1.0/xep-osw-profile.html
     * @return true if it succeeded in posting the profile to the server (i.e. if it receives a
     *   reply IQ of type RESULT.)
     * @throws RequestException throws a RequestException if it receives from the server a reply IQ
     *   of type ERROR after trying to set the Profile.
     * @throws AuthenticationRequired throws an AuthenticationRequired is no user has been logged in
     *   yet.
     * @throws ConnectionRequired throws a ConnectionRequired exception if not connected to an XMPP
     *   Server.
    ...
    public boolean setProfile(Profile profile)
        throws RequestException, AuthenticationRequired, ConnectionRequired;

    /* Relation related commands */

    /**
     * Retrieves from the XMPP Server the List of Relations of a user by sending an IQ of
     * type IQRelationQuery
     *
     ...
     * @throws RequestException throws a RequestException if it receives from the server a reply
     *   IQ of type ERROR after trying to retrieve the Relations
     * @throws ConnectionRequired throws a ConnectionRequired exception if not connected to an XMPP
     *   Server.
     * @throws AuthenticationRequired throws an AuthenticationRequired is no user has been logged in
     *   yet.
     */
    public List<Relation> getRelations(String userJid)
        throws RequestException, ConnectionRequired, AuthenticationRequired;
     ...
     * @throws RequestException throws a RequestException if it receives from the server
     *   a reply IQ of type ERROR after trying to add the Relations.
     * @throws AuthenticationRequired throws an AuthenticationRequired is no user has been
     *   logged in yet.
     * @throws ConnectionRequired throws a ConnectionRequired exception if not connected to
     *   an XMPP Server.
     */
     ...
     * @return true if no error ocurred.
     * @throws RequestException throws a RequestException if it receives from the server a
     *   reply IQ of type ERROR after trying to add the Relation.
     * @throws AuthenticationRequired throws an AuthenticationRequired is no user has been
     *   logged in yet.
     * @throws ConnectionRequired throws a ConnectionRequired exception if not connected to
     *   an XMPP Server.
    ...
    /**
     * Send to the XMPP Server a Presence Stanza containing a new type of Presence for this
     * user+resource
     *
     ...
     *   available  (the default), chat, away, xa  (extended away), and dnd  (do not disturb).
     */
    public void setPresenceMode(Type pres, Mode mod);

    /**
     * Obtains the presence of the current user (if the same user as connected is specified),
     * else it will try to obtain the presence information of the specified user from the roster.
     *
    ...
    public String getUploadToken(String requestID)
        throws RequestException, AuthenticationRequired, ConnectionRequired;
}
